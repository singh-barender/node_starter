...
const signin = async (req: Request<Record<string, never>, unknown, SigninRequestBody>, res: Response): Promise<void> => {
  // Check if the user exists and account is active
  const user = await findUserByEmailOrUsername(req.body.email);
